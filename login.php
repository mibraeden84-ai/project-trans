...
function safeLoginRedirect(?string $target, string $fallback): string {
...
function defaultRedirectForRole(string $role): string {
    return in_array($role, ['editor', 'admin'], true) ? 'admin/dashboard.php' : 'index.php';
}

function redirectForRole(string $role, ?string $target): string {
    $fallback = defaultRedirectForRole($role);
    $redirect = safeLoginRedirect($target, $fallback);
    $normalized = ltrim(str_replace('\\', '/', $redirect), '/');
    $isAdminPath = str_starts_with($normalized, 'admin/');

    if (in_array($role, ['editor', 'admin'], true)) {
        return $isAdminPath ? $redirect : $fallback;
    }

    return $isAdminPath ? $fallback : $redirect;
}

if (isLoggedIn()) {
    $role = $_SESSION['user_role'] ?? '';
    $redirect = redirectForRole($role, $_GET['redirect'] ?? '');
