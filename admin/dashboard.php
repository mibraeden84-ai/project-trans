...
if (!canManageFiles()) {
    $target = isLoggedIn() ? '../index.php' : '../login.php?redirect=' . urlencode('admin/dashboard.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
    ...
    if (isset($uploadSubdirs[$uploadType]) && isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($uploadType, ['config', 'firmware', 'manual', 'software'], true) && !$modelId) {
            $typeLabel = [
                'config' => 'config files',
            ...
            $uploadRespond(false, 'Model is required for ' . $typeLabel);
        }
        if ($modelId) {
        ...
        <div id="tab-add-file" class="tab-content <?= $initialTab === 'add-file' ? 'active' : '' ?>">
            ...
            <script>
                ...
                function initUploadAsync() {
                    ...
                    function uploadChunk(uploadIdVal, file, start, end, index, total, metadata) {
                        var chunk = file.slice(start, end);
                        var formData = new FormData();
                        formData.append('chunk', chunk, file.name);
                        formData.append('upload_id', uploadIdVal);
                        formData.append('chunk_index', String(index));
                        formData.append('total_chunks', String(total));
                        if (index === 0) {
                            formData.append('upload_type', metadata.upload_type || 'config');
                            formData.append('display_name', metadata.display_name || '');
                            formData.append('brand_id', metadata.brand_id || '');
                            ...
                            formData.append('system_type', metadata.system_type || '');
                        }
                        return new Promise(function(resolve, reject) {
                            var xhr = new XMLHttpRequest();
                            xhr.open('POST', '../chunk_upload.php?action=upload', true);
                            xhr.onload = function() {
                    ...
                    uploadForm.addEventListener('submit', function(e) {
                        ...
                        if ((type === 'config' || type === 'firmware' || type === 'manual' || type === 'software') && (!model || !model.value)) {
                            var modelMessage = type === 'firmware'
                                ? 'Please select the real device model for firmware files.'
                                : (type === 'manual'
                                    ? 'Please select the real device model for manuals.'
                                    : (type === 'software'
                                        ? 'Please select the real device model for software files.'
                                        : 'Please select a device model for configuration files.'));
                            if (typeof showToast === 'function') showToast(modelMessage, 'error', 4200);
                            return;
                        }

                        setUploadBusy(true);
                        setUploadStatus('', false);
                        setUploadProgress(0, 'Splitting file into chunks...');
                        ...
                        var metadata = {
                            upload_type: type,
                            display_name: document.querySelector('input[name="display_name"]') ? document.querySelector('input[name="display_name"]').value : '',
                            brand_id: brand ? brand.value : '',
                            model_id: model ? model.value : '',
                            version: document.querySelector('input[name="version"]') ? document.querySelector('input[name="version"]').value : '1.0',
                            description: document.querySelector('textarea[name="description"]') ? document.querySelector('textarea[name="description"]').value : '',
                            changelog: document.querySelector('textarea[name="changelog"]') ? document.querySelector('textarea[name="changelog"]').value : '',
                ...
                <table>
                    ...
                    <?php foreach ($allFirmware as $f): ?>
                        <tr>
                            ...
                            <td><?= escape($f['name']) ?></td>
                            <td><?= escape($f['brand_name']) ?> &mdash; <?= escape($f['model_name'] ?? 'No model selected') ?></td>
                ...
                <table>
                    ...
                    <?php foreach ($allManuals as $m): ?>
                        <tr>
                            ...
                            <td><?= escape($m['name']) ?></td>
                            <td><?= escape($m['brand_name']) ?> &mdash; <?= escape($m['model_name'] ?? 'No model selected') ?></td>
                ...
                <table>
                    ...
                    <?php foreach ($allSoftware as $s): ?>
                        <tr>
                            ...
                            <td><?= escape($s['name']) ?></td>
                            <td><?= escape($s['brand_name']) ?> &mdash; <?= escape($s['model_name'] ?? 'No model selected') ?></td>
