...
class FileController
{
    ...
    public function upload(Request $req, Response $res): Response
    {
        ...
        $modelId = (int)$req->input('model_id');
        if ($brandId <= 0) {
            return $res->error('Brand is required', 422);
        }
        if ($modelId <= 0) {
            return $res->error('Model is required', 422);
        }

        $db = Database::getInstance();
        $modelRow = $db->fetchOne("SELECT id, brand_id FROM device_models WHERE id = ?", [$modelId]);
        if (!$modelRow) {
            return $res->error('Selected model does not exist', 422);
        }
        if ((int)$modelRow['brand_id'] !== $brandId) {
            return $res->error('Selected model does not belong to selected brand', 422);
        }
            ...
            switch ($type) {
                case 'config':
                    $db->insert(
                        ...
                        ['config', $modelId, $name . '.' . $ext, $req->input('system_type'), $relativePath, $file['size'], $req->input('version', '1.0'), $req->input('description')]
                    );
                    break;
                ...
                case 'firmware':
                    $db->insert(
                        ...
                        [$brandId, $modelId, $name . '.' . $ext, $req->input('system_type'), $relativePath, $file['size'], $req->input('version', '1.0'), $req->input('changelog')]
                    );
                    break;
                ...
                case 'manual':
                    $db->insert(
                        ...
                        [$brandId, $modelId, $name . '.' . $ext, $req->input('system_type'), $relativePath, $file['size'], $req->input('description')]
                    );
                    break;
                ...
                case 'software':
                    $db->insert(
                        ...
                        [$brandId, $modelId, $name . '.' . $ext, $req->input('system_type'), $relativePath, $file['size'], $req->input('version', '1.0'), $req->input('description')]
                    );
                    break;
            }
        } catch (\Throwable $e) {
